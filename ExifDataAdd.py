...
class ImageApp(tk.Tk):
    def __init__(self):
        ...
        self.thumb = None
        self.thumb_white = ImageTk.PhotoImage(
            Image.new('RGB', (800, 450), color='white'))
        ...
        self.dir_label = tk.Label(
            self.image_frame, text="Please Load A Directory")
        self.image_label = tk.Label(
            self.image_frame, width=800, height=450, image=self.thumb_white)
        # Information messages to user
        self.info_label = tk.Label(
            self.image_frame, text="Information Appears Here")
        ...
        self.year_label = tk.Label(
            self.input_frame, text="Enter Year: (YYYY):")
        self.month_label = tk.Label(
            self.input_frame, text="Enter Month: (MM):")
        ...
        self.prev_button = tk.Button(
            self.button_frame, text="Prev", command=self.prev_image)
        self.load_button = tk.Button(
            self.button_frame, text="Select Read Directory",
            command=self.pick_read_dir)
        self.load2_button = tk.Button(
            self.button_frame, text="Select Save Directory",
            command=self.pick_save_dir)
        self.save_button = tk.Button(
            self.button_frame, text="Write Exif", command=self.write_exif)
        ...
        if (self.image_loaded):
            if (self.file_index != len(self.files)-1):
                ...
                self.info_label.config(
                    text="Reached End of Images, Sending to Start")
    ...
    def prev_image(self):
        ...
        if (self.image_loaded):
            ...
            if (self.file_index != 0):
                ...
                self.info_label.config(
                    text="Reached Beginning of Images, Sending to End")
    ...
    def pick_read_dir(self):
        ...
        self.dir_read_path = filedialog.askdirectory(
            title="Pick A Directory to Read Files From")
        if self.dir_read_path:
            self.files = self.run_fast_scandir(
                self.dir_read_path, [".jpg", ".jpeg", "JPG", ".JPEG"])[1]
            if (self.files):
    ...
    def pick_save_dir(self):
        ...
        self.dir_save_path = filedialog.askdirectory(
            title="Pick A Directory To Save Files To")
    ...
    def write_exif(self):
        if (not self.dir_read_path_good):
            self.info_label.config(
                text="Please Select A Directory to Read From")
            return
        ...
        if (not self.verify_day(day) or not self.verify_month(month)
                or not self.verify_year(year)):
        ...
        if (len(self.files) != 0):
            ...
            self.clear_read_dir()
            self.info_label.config(
                text="Photos Completed Please Select a New Read Directory")
